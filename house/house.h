...
public:
    ...
    // Physics simulation
    void applyForces(const std::vector<Balloon*>& balloons, WindSystem* windSystem = nullptr);
...
private:
    ...
    static constexpr float TILT_RESPONSE = 0.15f;      // How quickly house tilts
    static constexpr float TILT_DAMPING = 3.0f;        // Angular damping
    static constexpr float MAX_TILT_ANGLE = 0.25f;     // Max tilt in radians (~14 degrees)
